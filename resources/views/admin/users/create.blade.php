...
@section('subtitle', 'Completa los datos para registrar un nuevo usuario')
  ...
  <form method="POST" action="{{ route('admin.users.store') }}">
    ...
    <div class="form-grid">
      ...
      <div class="form-group">
        ...
        <input class="form-input" type="text" name="name" value="{{ old('name') }}" placeholder="Ej: Laura García" required/>
        @error('name') <span class="form-error">{{ $message }}</span> @enderror
      </div>

      <div class="form-group">
        <label class="form-label">Correo electrónico</label>
        <input class="form-input" type="email" name="email" value="{{ old('email') }}" placeholder="usuario@empresa.com" required/>
        @error('email') <span class="form-error">{{ $message }}</span> @enderror
      </div>

      <div class="form-group">
        <label class="form-label">Contraseña</label>
        <input class="form-input" type="password" name="password" placeholder="Mínimo 6 caracteres" required/>
      ...
      <div class="form-group">
        ...
        <input class="form-input" type="date" name="contract_start" value="{{ old('contract_start') }}"/>
        @error('contract_start') <span class="form-error">{{ $message }}</span> @enderror
      </div>

      <div class="form-group">
        <label class="form-label">Fecha fin contrato</label>
        <input class="form-input" type="date" name="contract_end" value="{{ old('contract_end') }}"/>
        @error('contract_end') <span class="form-error">{{ $message }}</span> @enderror
      </div>

      <div class="form-group">
        <label class="form-label">Sueldo</label>
        <input class="form-input" type="number" name="salary" value="{{ old('salary') }}" placeholder="0.00" step="0.01" min="0"/>
        @error('salary') <span class="form-error">{{ $message }}</span> @enderror
      </div>

      <div class="form-group">
        <label class="form-label">Bono de movilidad</label>
        <input class="form-input" type="number" name="mobility_bonus" value="{{ old('mobility_bonus') }}" placeholder="0.00" step="0.01" min="0"/>
